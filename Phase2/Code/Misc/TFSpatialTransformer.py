...
# ==============================================================================
'''
Modified provided transformer for tf2
'''
...
def transformer(U, theta, out_size, name='SpatialTransformer', **kwargs):
    ...
    def _meshgrid(height, width, scale_h):
        with tf.compat.v1.variable_scope('_meshgrid'):
            if scale_h:
                ...
                x_t = tf.matmul(tf.ones(shape=tf.stack([height, 1])),
                            tf.transpose(a=tf.expand_dims(tf.linspace(0.0, tf.cast(width,'float32'), width), 1), perm=[1, 0]))
    ...
    def _transform(theta, input_dim, out_size, scale_h):
        with tf.compat.v1.variable_scope('_transform'):
            ...
            condition =  tf.reduce_sum(input_tensor=tf.cast(tf.greater(tf.abs(t_s_flat), small),tf.float32))
